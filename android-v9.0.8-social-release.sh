...
# V9.0.8 changes social web UX/cache and the social Apps Script source only.
...
sed -i "s/versionCode 901/versionCode 903/; s/versionName '9.0.6'/versionName '9.0.8'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.8'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 903" "$GRADLE"
grep -q "versionName '9.0.8'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.8"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.8' "$PKG"

# Release source must be coherent before an Android artifact can be accepted.
grep -Fq "const APP_VERSION = 'V9.0.8';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.8';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v908" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.8"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.8 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.8 release source + Android 903 + native engine checks PASS'
