...
# 1) Version and stable app identity.
sed -i "s/versionCode 800/versionCode 802/; s/versionName '8.0'/versionName '8.0.2'/" "$SRC/app/build.gradle"

# 2) Restore the PWABuilder/Bubblewrap shell class names used by the working APK.
...
# 3) Native reminder activities should return to the restored launcher.
...
# 4) Restore the working PWABuilder/Bubblewrap TWA manifest shape, then add only native reminders.
...
# 5) The TWA shell itself uses a translucent framework theme; this style is only for the native settings screen.
...
# 6) Restore the exact web-app icon instead of the temporary diamond vector.
cp "$GITHUB_WORKSPACE/icon-512.png" "$RES/drawable/ic_launcher.png"

# Fail the build if the old launcher/query accidentally survives.
! grep -R "MainActivity" -n "$SRC/app/src/main" || exit 1
! grep -R "?native=1" -n "$SRC/app/src/main" || exit 1
grep -q "versionCode 802" "$SRC/app/build.gradle"
