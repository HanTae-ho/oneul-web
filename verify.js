...
ok(/id="social-leave-confirm"/.test(index)&&!/social-leave-next/.test(index)&&!/social-leave-word/.test(index),'커뮤니티 탈퇴 1회 확인 후 바로 처리');
...
ok(/SOCIAL_FEED_CACHE_KEY='ohg\.social\.feed\.v1'/.test(index)&&/function socialFeedCacheLoad\(sort\)/.test(index)&&/function socialFeedCacheSave\(sort,items\)/.test(index),'커뮤니티 피드 기기 캐시 선표시');
...
ok(/AUTH_CACHE_SECONDS\s*=\s*120/.test(read('social-apps-script.gs'))&&/FEED_TAIL_ROWS\s*=\s*400/.test(read('social-apps-script.gs'))&&/SUPPORT_CACHE_SECONDS\s*=\s*600/.test(read('social-apps-script.gs')),'커뮤니티 서버 V9.0.7 캐시/끝행 최적화');
ok(/function authCacheKey_\(/.test(read('social-apps-script.gs'))&&/function commentCounts_\(/.test(read('social-apps-script.gs'))&&/function profileActivity_\(b\)/.test(read('social-apps-script.gs')),'커뮤니티 서버 인증·댓글수·프로필 캐시 경로');
...
// 릴리스 버전은 상단 build/appVersion/cache 검증과 release-version-check.js에서 동적으로 확인합니다.
