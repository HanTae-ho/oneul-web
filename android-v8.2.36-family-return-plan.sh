...
# Rebuild the proven V8.2.35 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.35-family-conversation.sh"
...
# V8.2.36 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 855" "$GRADLE"
grep -q "versionName '8.2.35'" "$GRADLE"
sed -i "s/versionCode 855/versionCode 856/; s/versionName '8.2.35'/versionName '8.2.36'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 856" "$GRADLE"
grep -q "versionName '8.2.36'" "$GRADLE"
...
echo 'V8.2.36 Android version-only patch PASS'
