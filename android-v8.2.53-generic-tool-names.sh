...
# Reuse the verified V8.2.52 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.52-selfcheck-card-guide.sh"
...
sed -i "s/versionCode 872/versionCode 873/; s/versionName '8.2.52'/versionName '8.2.53'/" "$GRADLE"

grep -q "versionCode 873" "$GRADLE"
grep -q "versionName '8.2.53'" "$GRADLE"
...
echo 'V8.2.53 version-only rebuild from verified V8.2.52 native stack: PASS'
