...
# V9.0.13 changes meaning-reflection web behavior only.
...
sed -i "s/versionCode 901/versionCode 908/; s/versionName '9.0.6'/versionName '9.0.13'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.13'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 908" "$GRADLE"
grep -q "versionName '9.0.13'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.13"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.13' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.13';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.13';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9013-meaning-urge-review-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.13"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.13 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.13 release source + Android 908 + native engine checks PASS'
