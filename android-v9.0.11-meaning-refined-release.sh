...
# V9.0.11 adds the meaning-reflection web feature only.
...
sed -i "s/versionCode 901/versionCode 906/; s/versionName '9.0.6'/versionName '9.0.11'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.11'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 906" "$GRADLE"
grep -q "versionName '9.0.11'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.11"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.11' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.11';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.11';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9011-meaning-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.11"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.11 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.11 release source + Android 906 + native engine checks PASS'
