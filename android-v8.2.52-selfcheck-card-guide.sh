...
# Reuse the verified V8.2.51 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.51-trail-screening-mindpro.sh"
...
grep -q "versionCode 871" "$GRADLE"
grep -q "versionName '8.2.51'" "$GRADLE"
sed -i "s/versionCode 871/versionCode 872/; s/versionName '8.2.51'/versionName '8.2.52'/" "$GRADLE"

grep -q "versionCode 872" "$GRADLE"
grep -q "versionName '8.2.52'" "$GRADLE"
...
echo 'V8.2.52 version-only rebuild from verified V8.2.51 native stack: PASS'
