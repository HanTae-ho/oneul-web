...
# Rebuild the proven V8.2.39 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.39-usability-consolidation-2.sh"
...
# V8.2.40 is a web copy-cleanup release only. Native code stays unchanged; package version only.
grep -q "versionCode 859" "$GRADLE"
grep -q "versionName '8.2.39'" "$GRADLE"
sed -i "s/versionCode 859/versionCode 860/; s/versionName '8.2.39'/versionName '8.2.40'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 860" "$GRADLE"
grep -q "versionName '8.2.40'" "$GRADLE"
...
echo 'V8.2.40 Android version-only patch PASS'
