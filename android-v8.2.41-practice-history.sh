...
# Rebuild the proven V8.2.40 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.40-copy-cleanup.sh"
...
# V8.2.41 changes web practice-history presentation only. Native code stays unchanged; package version only.
grep -q "versionCode 860" "$GRADLE"
grep -q "versionName '8.2.40'" "$GRADLE"
sed -i "s/versionCode 860/versionCode 861/; s/versionName '8.2.40'/versionName '8.2.41'/" "$GRADLE"
...
echo 'V8.2.41 Android version-only patch PASS'
