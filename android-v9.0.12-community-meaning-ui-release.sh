...
# V9.0.12 changes user-visible community wording and meaning-reflection UI only.
...
sed -i "s/versionCode 901/versionCode 907/; s/versionName '9.0.6'/versionName '9.0.12'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.12'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 907" "$GRADLE"
grep -q "versionName '9.0.12'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.12"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.12' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.12';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.12';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9012-community-meaning-ui-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.12"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.12 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.12 release source + Android 907 + native engine checks PASS'
