...
# Rebuild the proven V8.2.41 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.41-practice-history.sh"
...
# V8.2.42 changes web practice-card presentation only. Native code stays unchanged; package version only.
grep -q "versionCode 861" "$GRADLE"
grep -q "versionName '8.2.41'" "$GRADLE"
sed -i "s/versionCode 861/versionCode 862/; s/versionName '8.2.41'/versionName '8.2.42'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 862" "$GRADLE"
grep -q "versionName '8.2.42'" "$GRADLE"
...
echo 'V8.2.42 Android version-only patch PASS'
