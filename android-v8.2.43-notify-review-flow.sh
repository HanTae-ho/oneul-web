...
# Rebuild the proven V8.2.42 native stack unchanged.
...
# V8.2.43 changes only web schedule/notification guidance. Native code stays unchanged; package version only.
grep -q "versionCode 862" "$GRADLE"
grep -q "versionName '8.2.42'" "$GRADLE"
sed -i "s/versionCode 862/versionCode 863/; s/versionName '8.2.42'/versionName '8.2.43'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 863" "$GRADLE"
grep -q "versionName '8.2.43'" "$GRADLE"
...
echo 'V8.2.43 Android version-only patch PASS'
