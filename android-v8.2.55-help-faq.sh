...
# Reuse the verified V8.2.54 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.54-smart-copy-completion.sh"
...
grep -q "versionCode 874" "$GRADLE"
grep -q "versionName '8.2.54'" "$GRADLE"
sed -i "s/versionCode 874/versionCode 875/; s/versionName '8.2.54'/versionName '8.2.55'/" "$GRADLE"

grep -q "versionCode 875" "$GRADLE"
grep -q "versionName '8.2.55'" "$GRADLE"
...
echo 'V8.2.55 version-only rebuild from verified V8.2.54 native stack: PASS'
