...
# V9.0.10 changes recovery-pattern / recovery-summary guidance folding only.
...
sed -i "s/versionCode 901/versionCode 905/; s/versionName '9.0.6'/versionName '9.0.10'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.10'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 905" "$GRADLE"
grep -q "versionName '9.0.10'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.10"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.10' "$PKG"

# Release source must be coherent before an Android artifact can be accepted.
grep -Fq "const APP_VERSION = 'V9.0.10';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.10';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9010" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.10"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.10 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.10 release source + Android 905 + native engine checks PASS'
