...
# Rebuild the proven V8.2.37 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.37-family-tools-hub.sh"
...
# V8.2.38 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 857" "$GRADLE"
grep -q "versionName '8.2.37'" "$GRADLE"
sed -i "s/versionCode 857/versionCode 858/; s/versionName '8.2.37'/versionName '8.2.38'/" "$GRADLE"
...
echo 'V8.2.38 Android version-only patch PASS'
