...
# Rebuild the proven V8.2.33 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.33-family-boundary.sh"
...
# V8.2.34 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 853" "$GRADLE"
grep -q "versionName '8.2.33'" "$GRADLE"
sed -i "s/versionCode 853/versionCode 854/; s/versionName '8.2.33'/versionName '8.2.34'/" "$GRADLE"
...
echo 'V8.2.34 Android version-only patch PASS'
