...
function openSmartThreeView(id){
  ...
  $('#three-to-plan').onclick=()=>{
    ...
    setTimeout(()=>openSmartChangePlanEditor(null,{change:r.future||'',reasons:r.gap||r.feelingAfter||'',steps:r.different||''}),60);
  };
...
old_sig="function openSmartChangePlanEditor(record){\n  const edit=!!record;\n"
new_sig="function openSmartChangePlanEditor(record,seed){\n  const edit=!!record; seed=seed||{};\n"
assert old_sig in s
s=s.replace(old_sig,new_sig,1)
s=s.replace("value=\"'+esc(edit?(record.change||''):'')+'\" placeholder=\"예: 술을 마시지 않는 생활을 시작하기\"", "value=\"'+esc(edit?(record.change||''):(seed.change||''))+'\" placeholder=\"예: 술을 마시지 않는 생활을 시작하기\"",1)
s=s.replace("<textarea id=\"cp-reasons\" maxlength=\"800\" placeholder=\"이유를 한 줄에 하나씩 적어도 좋습니다.\">'+esc(edit?(record.reasons||''):'')+'</textarea>", "<textarea id=\"cp-reasons\" maxlength=\"800\" placeholder=\"이유를 한 줄에 하나씩 적어도 좋습니다.\">'+esc(edit?(record.reasons||''):(seed.reasons||''))+'</textarea>",1)
s=s.replace("<textarea id=\"cp-steps\" maxlength=\"1200\" placeholder=\"예:\\n1. 오늘 집에 있는 술을 치운다.\\n2. 이번 주 모임에 한 번 참여한다.\\n3. 외래 예약을 지킨다.\">'+esc(edit?(record.steps||''):'')+'</textarea>", "<textarea id=\"cp-steps\" maxlength=\"1200\" placeholder=\"예:\\n1. 오늘 집에 있는 술을 치운다.\\n2. 이번 주 모임에 한 번 참여한다.\\n3. 외래 예약을 지킨다.\">'+esc(edit?(record.steps||''):(seed.steps||''))+'</textarea>",1)
