...
# Preserve navigation origin in browser history so SMART tool back labels match actual entry route.
...
# All existing SMART worksheet headers get a common dynamic back marker.
...
# Add common route-aware label sync before SMART hub code.
...
# Call label sync after current page changes.
old_cur = "  cur = p;\n  $('#tabs').style.display = (p === 'intro') ? 'none' : 'flex';"
assert old_cur in s
s = s.replace(old_cur, "  cur = p;\n  syncSmartBack();\n  $('#tabs').style.display = (p === 'intro') ? 'none' : 'flex';", 1)

# Fix bottom-tab ownership for all existing/new SMART pages.
...
page_anchor = '''</section>\n\n<!-- ══════════ 미래의 나에게 V8.2.0 ══════════ -->'''
# choose the final occurrence immediately preceding Future Self
...
# Replace Point 3 placeholder with real ABC entry and keep future tools noted.
...
# ABC functions inserted before learningAction.
...
# Update learning data Point 3 with an ABC action.
