...
# Rebuild the proven V8.2.31 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.31-smart-integration.sh"
...
# V8.2.32 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 851" "$GRADLE"
grep -q "versionName '8.2.31'" "$GRADLE"
sed -i "s/versionCode 851/versionCode 852/; s/versionName '8.2.31'/versionName '8.2.32'/" "$GRADLE"
...
echo 'V8.2.32 Android version-only patch PASS'
