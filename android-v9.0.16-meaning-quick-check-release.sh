...
# V9.0.16 adds web-only meaning recovery quick check and help/manual updates.
...
sed -i "s/versionCode 901/versionCode 911/; s/versionName '9.0.6'/versionName '9.0.16'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.16'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 911" "$GRADLE"
grep -q "versionName '9.0.16'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.16"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.16' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.16';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.16';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9016-meaning-quick-check-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.16"' "$GITHUB_WORKSPACE/latest-release.json"
...
EXPECTED_APP_VERSION=V9.0.16 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.16 release source + Android 911 + native engine checks PASS'
