...
# Rebuild the proven V8.2.36 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.36-family-return-plan.sh"
...
# V8.2.37 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 856" "$GRADLE"
grep -q "versionName '8.2.36'" "$GRADLE"
sed -i "s/versionCode 856/versionCode 857/; s/versionName '8.2.36'/versionName '8.2.37'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 857" "$GRADLE"
grep -q "versionName '8.2.37'" "$GRADLE"
...
echo 'V8.2.37 Android version-only patch PASS'
