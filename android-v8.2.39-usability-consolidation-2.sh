...
# Rebuild the proven V8.2.38 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.38-usability-consolidation.sh"
...
# V8.2.39 is web usability consolidation only. Native code stays unchanged; package version only.
grep -q "versionCode 858" "$GRADLE"
grep -q "versionName '8.2.38'" "$GRADLE"
sed -i "s/versionCode 858/versionCode 859/; s/versionName '8.2.38'/versionName '8.2.39'/" "$GRADLE"
...
echo 'V8.2.39 Android version-only patch PASS'
