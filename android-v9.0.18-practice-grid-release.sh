...
# V9.0.18 changes web-only recovery tools practice grid layout.
...
sed -i "s/versionCode 901/versionCode 913/; s/versionName '9.0.6'/versionName '9.0.18'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.18'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 913" "$GRADLE"
grep -q "versionName '9.0.18'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.18"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.18' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.18';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.18';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9018-practice-grid-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.18"' "$GITHUB_WORKSPACE/latest-release.json"
...
EXPECTED_APP_VERSION=V9.0.18 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.18 release source + Android 913 + native engine checks PASS'
