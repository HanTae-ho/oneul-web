...
# Rebuild the proven V8.2.34 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.34-family-navigation.sh"
...
# V8.2.35 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 854" "$GRADLE"
grep -q "versionName '8.2.34'" "$GRADLE"
sed -i "s/versionCode 854/versionCode 855/; s/versionName '8.2.34'/versionName '8.2.35'/" "$GRADLE"
...
echo 'V8.2.35 Android version-only patch PASS'
