...
# V9.0.15 changes meaning-record linkage and view behavior only.
...
sed -i "s/versionCode 901/versionCode 910/; s/versionName '9.0.6'/versionName '9.0.15'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.15'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 910" "$GRADLE"
grep -q "versionName '9.0.15'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.15"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.15' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.15';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.15';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9015-meaning-linked-urge-view-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.15"' "$GITHUB_WORKSPACE/latest-release.json"
grep -Fq "const SOCIAL_VERSION = 'V9.0.7-social-1';" "$GITHUB_WORKSPACE/social-apps-script.gs"
EXPECTED_APP_VERSION=V9.0.15 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.15 release source + Android 910 + native engine checks PASS'
