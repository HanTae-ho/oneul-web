...
# V9.0.6 changes web/social UX and Apps Script performance only.
# Reuse the proven V9.0.5 native reminder/TTS stack and advance package metadata.
bash "$GITHUB_WORKSPACE/android-v9.0.5-update-release.sh"
...
sed -i "s/versionCode 900/versionCode 901/; s/versionName '9.0.5'/versionName '9.0.6'/" "$GRADLE"

python3 - <<'PY'
from pathlib import Path
import os
root=Path(os.environ['GITHUB_WORKSPACE'])/'android-v8/app/src/main'
old='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.5'
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.6'
changed=[]
for p in root.rglob('*'):
...
if not changed:
    raise SystemExit('V9.0.5 native.html appv marker target not found')
print('appv marker files:',', '.join(changed))
PY

grep -q "versionCode 901" "$GRADLE"
grep -q "versionName '9.0.6'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.6"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.6' "$PKG"
...
echo 'V9.0.6 Android versionCode 901 + appv marker release patch PASS'
