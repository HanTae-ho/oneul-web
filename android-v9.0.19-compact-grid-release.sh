...
# V9.0.19 changes web-only recovery tools card density/naming.
...
sed -i "s/versionCode 901/versionCode 914/; s/versionName '9.0.6'/versionName '9.0.19'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.19'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 914" "$GRADLE"
grep -q "versionName '9.0.19'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.19"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.19' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.19';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.19';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9019-compact-grid-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.19"' "$GITHUB_WORKSPACE/latest-release.json"
...
EXPECTED_APP_VERSION=V9.0.19 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.19 release source + Android 914 + native engine checks PASS'
