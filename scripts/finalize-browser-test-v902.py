...
one("""  await pg.click('#rec-wb4'); await pg.waitForTimeout(120);""", """  await pg.evaluate(() => openWorkbook('step4','rec')); await pg.waitForTimeout(120);""", 'step4 navigation')
one("""  await pg.click('#wb-save-record'); await pg.waitForTimeout(150);
...
one("""  await pg.click('#screen-sex-m'); await pg.waitForTimeout(100);
  for(let i=0;i<10;i++){
...
"""  await pg.click('#screen-sex-m'); await pg.waitForTimeout(100);
  ...
  assert((await pg.$eval('.screen-score .n', e => e.innerText)) === '0', 'AUDIT-K 0점 결과');""", 'AUDIT first run')

one("""  await pg.click('#screen-sex-m'); await pg.waitForTimeout(80);
  for(let i=0;i<10;i++){
...
"""  await pg.click('#screen-sex-m'); await pg.waitForTimeout(80);
  ...
  await pg.waitForTimeout(80);
  const secondAudit = await pg.$eval('#screen-test-body', e => e.innerText);""", 'AUDIT second run')

p.write_text(s,encoding='utf-8')
print('browser test current family/workbook/AUDIT flow PASS')
