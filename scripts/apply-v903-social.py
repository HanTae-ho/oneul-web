...
function feedPublicSnapshot_(sort){
  ...
  const counts={}; comments.forEach(function(r){const id=str_(r.postId);counts[id]=(counts[id]||0)+1;});
...
function invalidateFeedCache_(userId){
  try{const c=CacheService.getScriptCache();c.remove('feed:'+SOCIAL_VERSION+':latest');c.remove('feed:'+SOCIAL_VERSION+':support');if(userId)c.remove('sup:'+SOCIAL_VERSION+':'+userId);}catch(ignore){}
}'''
sub(r'function feed_\(p\)\{.*?\n\}\n\n/\* ─────────────────────────────\n \* 프로필',feed+'\n\n/* ─────────────────────────────\n * 프로필','feed')
...
function profileActivity_(b){
  ...
  const profile=rows_(SOCIAL_SHEETS.profiles).find(function(r){return str_(r.userId)===me.userId&&str_(r.status)==='active';});if(!profile)return {ok:false,error:'AUTH'};
  ...
  const myIds=new Set(myPosts.map(function(r){return str_(r.postId);})),postById={};allPosts.forEach(function(r){postById[str_(r.postId)]=r;});
...
sub(r'function profile_\(b\)\{.*?\n\}\n\nfunction profileDelete_',profile+'\n\nfunction profileDelete_','profile')
...
one('function commentList_(b){\n  ensureSheets_();\n  const me = auth_(b.userId,b.token);','function commentList_(b){\n  ensureSheets_();\n  const me = auth_(b.userId,b.token,true);','commentList ensure')

ensure=r'''function schemaCacheKey_(){ return 'schema:' + SOCIAL_VERSION; }
function ensureSheets_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet();if(!ss)throw new Error('NO_SPREADSHEET');
  let cache=null;try{cache=CacheService.getScriptCache();if(cache.get(schemaCacheKey_())==='1')return;}catch(ignore){cache=null;}
  Object.keys(SOCIAL_HEADERS).forEach(function(name){const sh=ss.getSheetByName(name);if(!sh)throw new Error('MISSING_SHEET:'+name);const heads=SOCIAL_HEADERS[name];const cur=sh.getRange(1,1,1,heads.length).getValues()[0].map(String);if(cur.join('|')!==heads.join('|'))throw new Error('BAD_HEADER:'+name);});
  if(cache){try{cache.put(schemaCacheKey_(),'1',SCHEMA_CACHE_SECONDS);}catch(ignore){}}
}'''
sub(r'function ensureSheets_\(\)\{.*?\n\}\n\n/\* 한 열의 여러 칸',ensure+'\n\n/* 한 열의 여러 칸','ensure cache')
...
# Invalidate feed cache after mutations.
repls=[
("  sh.getRange(profile._row,6).setValue('deleted');\n\n  return {ok:true};","  sh.getRange(profile._row,6).setValue('deleted');\n\n  invalidateFeedCache_(me.userId);\n  return {ok:true};"),
("  return {ok:true,id:id};\n}\n\nfunction postEdit_","  invalidateFeedCache_(me.userId);\n  return {ok:true,id:id};\n}\n\nfunction postEdit_"),
("  sh.getRange(r._row,6).setValue(Date.now());\n\n  return {ok:true};\n}\n\nfunction postDelete_","  sh.getRange(r._row,6).setValue(Date.now());\n\n  invalidateFeedCache_(me.userId);\n  return {ok:true};\n}\n\nfunction postDelete_"),
("    });\n\n  return {ok:true};\n}\n\n/* ─────────────────────────────\n * 댓글","    });\n\n  invalidateFeedCache_(me.userId);\n  return {ok:true};\n}\n\n/* ─────────────────────────────\n * 댓글"),
("  return {ok:true,id:id,commentCount:count};","  invalidateFeedCache_(me.userId);\n  return {ok:true,id:id,commentCount:count};"),
("  return {ok:true,commentCount:count};","  invalidateFeedCache_(me.userId);\n  return {ok:true,commentCount:count};"),
("  return {ok:true,supported:on,supportCount:count};","  invalidateFeedCache_(me.userId);\n  return {ok:true,supported:on,supportCount:count};")]
for i,(a,b) in enumerate(repls): one(a,b,'invalidate '+str(i))
