...
# Rebuild the proven V8.2.32 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.32-family-integration.sh"
...
# V8.2.33 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 852" "$GRADLE"
grep -q "versionName '8.2.32'" "$GRADLE"
sed -i "s/versionCode 852/versionCode 853/; s/versionName '8.2.32'/versionName '8.2.33'/" "$GRADLE"

# Guardrails: relaxation TTS and all reminder engines must remain intact.
grep -q "versionCode 853" "$GRADLE"
grep -q "versionName '8.2.33'" "$GRADLE"
...
echo 'V8.2.33 Android version-only patch PASS'
