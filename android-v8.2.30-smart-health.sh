...
# Rebuild the proven V8.2.29 native stack unchanged.
bash "$GITHUB_WORKSPACE/android-v8.2.29-relax-rate.sh"
...
# V8.2.30 is web-only. Native code stays unchanged; package version only.
grep -q "versionCode 849" "$GRADLE"
grep -q "versionName '8.2.29'" "$GRADLE"
sed -i "s/versionCode 849/versionCode 850/; s/versionName '8.2.29'/versionName '8.2.30'/" "$GRADLE"
...
echo 'V8.2.30 Android version-only patch PASS'
