...
# V9.0.17 changes web-only meaning-check access/history UX.
...
sed -i "s/versionCode 901/versionCode 912/; s/versionName '9.0.6'/versionName '9.0.17'/" "$GRADLE"
...
new='https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.17'
changed=[]
for p in root.rglob('*'):
...
grep -q "versionCode 912" "$GRADLE"
grep -q "versionName '9.0.17'" "$GRADLE"
grep -Fq 'android:value="https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.17"' "$MANIFEST"
grep -R -Fq 'https://hantae-ho.github.io/oneul-web/native.html?appv=9.0.17' "$PKG"

grep -Fq "const APP_VERSION = 'V9.0.17';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq "const BUILD='V9.0.17';" "$GITHUB_WORKSPACE/index.html"
grep -Fq "const V = 'ohg-v9017-meaning-check-ux-r1';" "$GITHUB_WORKSPACE/sw.js"
grep -Fq '"version": "V9.0.17"' "$GITHUB_WORKSPACE/latest-release.json"
...
EXPECTED_APP_VERSION=V9.0.17 ANDROID_GRADLE="$GRADLE" node "$GITHUB_WORKSPACE/release-version-check.js"
...
echo 'V9.0.17 release source + Android 912 + native engine checks PASS'
